...
export class InvoicereportService {
  ...
  getInvoiceReport(fromDate: string, toDate: string, branchId: any) {
    const form = 'X_API_KEY=' + localStorage.getItem('userToken') +
      '&user_id=' + localStorage.getItem('userId') +
      '&fromDate=' + fromDate +
      '&toDate=' + toDate +
      '&branchId=' + (Array.isArray(branchId) ? branchId.join(',') : branchId);
    return this.http.post(this.reportsUrl + 'api/reports/invoice_report', form, { headers: this.reqHeader });
  }
}
